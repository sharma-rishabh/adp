...
3. Call `read_file` on `todos.md` (open action items), `goals.md` (long-term goals), and `preferences.md`
...
6. **Suggest ONE specific action based on what the schedule says is free:**
   ...
   - Match it to the user's goals (from `goals.md`) and preferences (from `preferences.md`)
...
7. Keep the nudge to 2-3 sentences max. During busy blocks, keep it to 1 sentence.
8. If you decide not to nudge, respond with ONLY `[skip]` — nothing else
9. Do NOT use any tool besides `get_current_datetime`, `get_today_schedule`, and `read_file` — no writes during nudges
